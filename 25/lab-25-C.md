...
# la salida de un objeto presente en 'etcd' en formato YAML o JSON. Así se puede tener 
...
# (Nota: Observar cómo K8s agrega sus propios parámetros)
...
# Comprobar que el registro A redis-master.default.svc.cluster.local. se resuelve a la CLUSTER-IP
# anterior. CTRL+C para salir.
...
# Para ello estudiamos el archivo 'lab-25-C-redis-replica-deployment.yaml'
...
#
# Líneas 8-12:  El deployment se asociará con una plantilla de pod que tenga definidas las etiquetas: 
#               'app: redis', 'role: replica' y 'tier: backend'.
#
...
# Ahora desplegamos en frontend, que se contectará al servidor máster de Redis o a las réplicas
# en función del tipo de consulta que necesite hacer.
...
#
# Línea 9-11:       Este deployment se asociará con la plantilla de pod que tiene definidas las etiquetas 
#                   'app: guestbook' y 'tier: frontend'.
#
...
#                   }
#
#                   Es decir, que se servidor máster de Redis será el que indique la variable del sistema 'REDIS_MASTER_SERVICE_HOST'.
#                   Esta variable la crea Kubernetes cuando se despliega el master.
#
# Líneas 29 y 30:   Se inicializa la variable de entorno 'REDIS_SLAVE_SERVICE_HOST' al valor 'redis-replica-internal-service', para 
#                   que los contenedores de Frontend puedan contactar con la réplicas.
