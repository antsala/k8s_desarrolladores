...
# Este laboratorio crearemos una aplicación de dos capas. Para ello crearemos un pod
# con dos contenedores. El contenedor de Frontend tendrá una imagen de 'phpMyAdmin', mientras
# que el contenedor de Backend usará una imagen de 'mySQL'. La información de usuario y contraseña
# se inyectará a los contenedores en tiempo de ejecución, mediante un archivo de variables de 
# entorno.
...
#
#   2)  Tener instalado el runtime de contenedor 'podmam', o lo que es lo mismo, haber realizado
#       el ejercici anterior (lab-06-A.txt)
...
# Este laboratorio desplegará un pod con dos contenedores. El Frontend con 'phpMyAdmin' y el 
# Backend con 'mySQL'. En primer lugar comprobamos que no existen contenedores en ejecución con
# podman.
...
# El Frontend de phpMySQL necesitará credenciales para conectar con 'mySQL'. Al mismo
# tiempo, a 'mySQL' debemos proporcionarle las credenciales de administrador. Todo ello
# se debe hacer mediante variables de entorno y está documentado en el repositorio de las
# imágenes.
#
# Para 'mySQL', la documentación está en: 'https://hub.docker.com/_/mysql'. 
# En el apartado 'Environment Variables', podemos leer...
# (Nota: ponemos fragmentos de la documentación)
#
# ...
# When you start the mysql image, you can adjust the configuration of the MySQL instance by passing one 
# or more environment variables on the docker run command line. 
# ...
# MYSQL_ROOT_PASSWORD
...
#
# Para 'phpMyAdmin', la documentación está en 'https://hub.docker.com/_/phpmyadmin'. 
# En el apartado 'Environment variables summary' podemos leer...
# (Nota: ponemos fragmentos de la documentación)
#
# ...
...
# La salida será similar a esta:
# (Nota: Se puede ver que existe el contenedor de infraestructura porque el número de contenedores es '1')
#
# POD ID        NAME           STATUS      CREATED         INFRA ID      # OF CONTAINERS
...
#   2)  Debe incluir el archivo de variables de entorno.
#
#   3)  Debemos inyectarle la variable de entorno 'PMA_HOST=127.0.0.1' para que la regla de nateo funcione sobre 
#       este contenedor.
...
# Podemos comprobar que el runtime no ha asignado una dirección IP al ninguno de los contenedores:
...
# (Nota: el parámetro '--pod' hace que se muestre información del pod al que pertenece el contenedor)
...
# Podemos observsar que hay tres reglas de nateo, una para el contenedor de infraestructura, otro para el Frontend y la 
# última para el Backend. Debería resultar extraño como se permite tráfico http a los contenedores de infraestructura y
# de la base de dato, pero no tiene ninguna implicación puesto que la regla sea válida se debe inyectar la variable de 
# entorno 'PMA_HOST=127.0.0.1' al contenedor que deseemos que reciba tráfico.
